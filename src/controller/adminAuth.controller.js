...
const setSalesmanCommission = async (req, res) => {
  ...
  try {
    ...
    if (subscriptionCommission !== undefined)
      settings.subscriptionCommission = subscriptionCommission;
...
const setManagerCommission = async (req, res) => {
  const {
    amount, // Manager's own sales commission
    salesTarget,
    subscriptionCommission,
    salesmanSalesCommissionForManager, // NEW: commission manager gets from salesman sales
  } = req.body;
  ...
  try {
    ...
    // Manager direct commission
    if (amount !== undefined) settings.salesCommission = amount;
    if (salesTarget !== undefined) settings.salesTarget = salesTarget;
    if (subscriptionCommission !== undefined)
      settings.subscriptionCommission = subscriptionCommission;

    // Manager's commission from sales generated by salesmen
    if (salesmanSalesCommissionForManager !== undefined) {
      settings.salesmanSalesCommissionForManager =
    ...
    res.status(200).json({
      message: "Manager commission settings updated successfully",
      updatedSettings: {
    ...
    res.status(500).json({
      message: "Error updating manager commission settings",
      error: err.message,
    });
  }
};
